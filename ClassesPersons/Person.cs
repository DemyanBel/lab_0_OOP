...
namespace ClassPerson
{
    ...
    public class Person
    {
        ...
        public static void CheckLanguage(string firstName, string lastName)
        {
            ...
            //TODO: duplication
            bool firstNameIsCyrillic = 
                (firstNameCategory == UnicodeCategory.LowercaseLetter 
                || firstNameCategory == UnicodeCategory.UppercaseLetter) &&
                (firstName[0] >= 'а' && firstName[0] <= 'я' 
                || firstName[0] >= 'А' && firstName[0] <= 'Я' 
                || firstName[0] == 'ё' || firstName[0] == 'Ё');
            
            //TODO: duplication
            bool lastNameIsCyrillic = 
                (lastNameCategory == UnicodeCategory.LowercaseLetter 
                || lastNameCategory == UnicodeCategory.UppercaseLetter) &&
                (lastName[0] >= 'а' && lastName[0] <= 'я' 
                || lastName[0] >= 'А' && lastName[0] <= 'Я' 
                || lastName[0] == 'ё' || lastName[0] == 'Ё');
        ...
        public static Person GetRandomPerson()
        {
            ...
            if (random.Next(100) < probabilityLastNameChance)
            {
                ...
                do
                {
                    secondLastName = isUsingCyrillic
                        ? (randomGender == Gender.Male
        ...
        private static string GetRandomValueFromList(Random random, string[] maleNamesCyrillic)
        {
            return maleNamesCyrillic[random.Next(maleNamesCyrillic.Length)];
        }
    }
}
